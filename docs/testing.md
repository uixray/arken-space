...
- `campaign-isolation-routes.test.ts` — Fastify `onRoute` собирает фактически
  зарегистрированные ID-маршруты, включая sub-router и программную регистрацию.
  Каждый путь обязан быть campaign-scoped либо входить в один из двух закрытых
  глобальных наборов. Для каждого campaign-пути с точным сегментом `:id`
  дополнительно обязателен исполняемый probe из двухкампанейной HTTP-матрицы.
...
Для нового серверного пути с `:id` или `:*Id` порядок механический:

1. добавить route key в campaign registry либо доказанное глобальное исключение;
2. для точного `:id` добавить валидный foreign-entity probe с актором другой
   кампании, ожидаемым `403/404` и проверкой отсутствия мутации;
3. если маршрут читает файл или имеет раннюю role/payload-проверку, добавить
   same-campaign control, чтобы такой ранний ответ не маскировал tenant defect.

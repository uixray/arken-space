...
const message = {
  playerRequestId: "11111111-1111-4111-8111-111111111111",
} as ChatMessageDto;
const request = {
  id: message.playerRequestId,
  revision: 3,
  status: "ACKNOWLEDGED",
} as PlayerRequestDto;
describe("resolvePlayerRequestCard", () => {
  it("resolves the latest canonical DTO", () =>
    expect(resolvePlayerRequestCard(message, [request])).toBe(request));
  it("returns unavailable without leaking an id", () =>
    expect(resolvePlayerRequestCard(message, [])).toBeUndefined());
  it("ignores ordinary chat messages", () =>
    expect(
      resolvePlayerRequestCard({} as ChatMessageDto, [request]),
    ).toBeNull());

  it("keeps localized source labels intact", () => {
    const source = readFileSync(
      new URL("./player-request-chat.tsx", import.meta.url),
      "utf8",
    );
    for (const label of [
      "Заявка недоступна",

...
export interface SceneRendererProps {
  ...
  /**
   * UIX-466 п. 4: мастер обвёл зону боя.
   *
   * Тот же драг прямоугольника, что и у `SCENE_REGION`, — отличается только
   * тем, куда уходит результат. Рендерер по-прежнему сообщает наружу мировой
   * прямоугольник и не знает, что с ним будет: сохранением занимается App.
   */
  ...
  /**
   * UIX-466 п. 4: сохранённая зона боя активной сцены — её мастер видит на
   * карте постоянно, а не только пока тянет рамку. `null` — зона не задана.
   *
   * Приезжает только мастеру (сервер не кладёт её в проекцию игрока), поэтому
   * рендерер рисует её без дополнительной проверки роли: чего нет в данных,
   * того он не покажет.
   */
  battleZone?: {
    x: number;

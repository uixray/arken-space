...
describe("humanizeFormula", () => {
  it("replaces a single stat key with its localized label", () => {
    expect(humanizeFormula("1d20 + agility", statLabels)).toBe(
      "1d20 + Ловкость",
    );
  });

  it("replaces every stat key when a formula references several", () => {
    expect(humanizeFormula("1d20 + strength + agility", statLabels)).toBe(
      "1d20 + Сила + Ловкость",
    );
  });

  it("replaces the same key when it repeats", () => {
    expect(humanizeFormula("agility + agility", statLabels)).toBe(
      "Ловкость + Ловкость",
    );
  });

  it("returns formulas with no stat tokens unchanged", () => {
    expect(humanizeFormula("1d20", statLabels)).toBe("1d20");
    expect(humanizeFormula("1d20 + 3", statLabels)).toBe("1d20 + 3");
    expect(humanizeFormula("2d6kh1", statLabels)).toBe("2d6kh1");
  });

  it("returns an empty string unchanged", () => {
    expect(humanizeFormula("", statLabels)).toBe("");
  });
  ...
  it("leaves unknown/unrecognized tokens untouched instead of crashing", () => {
    ...
    expect(humanizeFormula("1d20 + endurance", statLabels)).toBe(
      "1d20 + endurance",
    );
    expect(humanizeFormula("1d20 + strength + endurance", statLabels)).toBe(
      "1d20 + Сила + endurance",
    );
  });
  ...
  it("is word-boundary safe: a key that is a substring of another word is left alone", () => {
    ...
    expect(humanizeFormula("1d20 + agilityBonus", statLabels)).toBe(
      "1d20 + agilityBonus",
    );
    expect(humanizeFormula("1d20 + superstrength", statLabels)).toBe(
      "1d20 + superstrength",
    );
    // and a real key still matches when it's a standalone token even if
    // another key is a prefix of it in spelling terms.
    expect(humanizeFormula("1d20 + reaction", statLabels)).toBe(
      "1d20 + Реакция",
    );
  });
  ...
  it("handles every stat key the system defines", () => {
    ...
    for (const stat of arkenSystem.stats)
      expect(humanizeFormula(`1d20 + ${stat.key}`, statLabels), stat.key).toBe(
        `1d20 + ${stat.label}`,
      );
  ...
  it("does not mangle dice notation or arithmetic operators", () => {
    expect(humanizeFormula("2d6 + strength - 1", statLabels)).toBe(
      "2d6 + Сила - 1",
    );
  });

  it("uses a label from a campaign-specific layout", () => {
    expect(
      humanizeFormula("1d20 + vnimatelnost", {
        vnimatelnost: "Внимательность",
      }),
    ).toBe("1d20 + Внимательность");
  });
});

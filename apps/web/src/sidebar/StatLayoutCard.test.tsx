...
describe("карточка группы характеристик", () => {
  ...
  it("показывает иконки действий без подмены доступных имён", () => {
    renderCard();
    const actionNames = [
      "Переместить «Ловкость» выше",
      "Переместить «Ловкость» ниже",
      "Переименовать «Ловкость»",
      "Удалить «Ловкость»",
      "Добавить строку",
    ];

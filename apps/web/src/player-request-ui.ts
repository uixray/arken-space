...
export const requestLabels = {
  ...
  horizon: {
    NOW: "Сейчас",
    BEFORE_BREAK: "До перерыва",
    NEXT_SESSION: "К следующей сессии",
  },
  status: {
...
export function visiblePlayerRequests(
  ...
  return requests.filter((request) => {
    if (role === "PLAYER" && request.authorMembershipId !== membershipId)
      return false;
    const open =
      request.status === "SUBMITTED" || request.status === "ACKNOWLEDGED";
    if (filters.state !== "ALL" && (filters.state === "OPEN") !== open)
      return false;
    if (filters.horizon !== "ALL" && request.horizon !== filters.horizon)
      return false;
...
export function createRequestPayload(input: {
  title: string;
  body: string;
  horizon: PlayerRequestHorizon;
  audience: PlayerRequestAudience;
  characterId: string;
}) {
  return {

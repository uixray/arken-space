...
import { canCancelRequest, canEditRequest, createRequestPayload, requestCharacters, requestLabels, visiblePlayerRequests } from "./player-request-ui";
...
const character = (id: string, overrides: Partial<CharacterDto> = {}): CharacterDto => ({
  id, name: id, ownerMembershipId: null, controllerMembershipIds: [], portraitAssetId: null,
  stats: {}, skills: [], spells: [], notes: "", backstory: "", inventory: [], resources: {},
  wallet: { gold: 0, silver: 0, copper: 0, sp: 0 }, entries: [], revision: 0, ...overrides,
});
...
describe("player request UI policy", () => {
  it("uses exact audience labels and trims create payload", () => {
    ...
    expect(createRequestPayload({ title: "T", body: "B", horizon: "NOW", audience: "PUBLIC", characterId: "delegated" }).characterId)
      .toBe("delegated");
  });
  it("offers owned, delegated-controlled, and active characters without leaking unrelated ones", () => {
    const characters = [
      ...
      character("active"),
      character("unrelated", { ownerMembershipId: "other", controllerMembershipIds: ["other"] }),
    ];

    expect(requestCharacters(characters, "member", "active").map(({ id }) => id))
      .toEqual(["owned", "delegated", "active"]);
    expect(requestCharacters([character("same", { ownerMembershipId: "member", controllerMembershipIds: ["member"] })], "member", "same"))
      .toHaveLength(1);
  });

...
describe("web source encoding", () => {
  ...
  it("contains no escaped Cyrillic or replacement placeholders in UI source", () => {
    ...
    const findings = files.flatMap((file) => {
      ...
      const matches =
        source.match(
          /\\u04[0-9a-f]{2}|\?{4,}|\u0413\u2014|\u0420 (?=[\u0430-\u044f])|"\?"\s*:\s*"\?"|\u0417\u0430\u0433\u0440\u0443\u0437\u043a\u0430\?| \? \u0432\u044b\u0431\u0440\u0430\u043d|join\(" \? "\)/g,
        ) ?? [];
      return matches.map(
        (match) =>
  ...
  it("keeps the workspace heading readable", () => {
    // UIX-472: маркером была подпись «Рабочее пространство» — заголовок
    // выпадающего списка разделов, который убрали вместе с самим списком.
    // Взята подпись панели инструментов: она у карты одна и живёт в разметке
    // независимо от того, как показывают разделы.
    const expected = [
      0x418, 0x43d, 0x441, 0x442, 0x440, 0x443, 0x43c, 0x435, 0x43d, 0x442,
      0x44b, 0x20, 0x43a, 0x430, 0x440, 0x442, 0x44b,
    ]
  ...
  it("keeps character and scene feedback copy readable", () => {
    const root = path.join(process.cwd(), "apps", "web", "src");
    // Sidebar.tsx was split into apps/web/src/sidebar/*.tsx per-domain
    // modules; these strings live in their extracted homes now.
    const characterWorkspace = readFileSync(
      path.join(root, "sidebar", "CharacterWorkspace.tsx"),
      "utf8",
    );
    const chatPanels = readFileSync(
      path.join(root, "sidebar", "ChatPanels.tsx"),
      "utf8",
    );
    const app = readFileSync(path.join(root, "App.tsx"), "utf8");
    expect(characterWorkspace).toContain(
      "\u0421\u043e\u0437\u0434\u0430\u0442\u044c \u043f\u0435\u0440\u0441\u043e\u043d\u0430\u0436\u0430",
    );
    expect(characterWorkspace).toContain(
      "\u0417\u0430\u043a\u0440\u044b\u0442\u044c \u0440\u0430\u0431\u043e\u0447\u0435\u0435 \u043f\u0440\u043e\u0441\u0442\u0440\u0430\u043d\u0441\u0442\u0432\u043e \u043f\u0435\u0440\u0441\u043e\u043d\u0430\u0436\u0435\u0439",
    );
    ...
    const catalog = readFileSync(
      path.join(root, "CatalogEntryForm.tsx"),
      "utf8",
    );
    const rollMode = readFileSync(
      path.join(root, "RollModeControl.tsx"),
      "utf8",
    );

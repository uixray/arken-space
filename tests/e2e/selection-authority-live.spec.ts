...
test("UIX-507 real mixed move rejects stale revisions atomically and respects revoked control", async ({
  page,
  browser,
  gmToken,
}, info) => {
  ...
  try {
    ...
    const drag = async () => {
      ...
      const start = frame.point(416, 352);
    ...
    expect(afterMove.token).toMatchObject({ x: 448, y: 320 });
    expect(afterMove.drawing).toMatchObject({ x: 576, y: 320 });
    expect(state(await snapshot(page))).toEqual(afterMove);
    await expect
      .poll(async () =>
        Number(await observer.map.getAttribute("data-resize-handle-x")),
      )
      .toBeCloseTo(beforeX + 64 * observer.fit.scale, 0);

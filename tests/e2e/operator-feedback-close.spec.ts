...
for (const mode of ["close", "revoked"] as const) {
  test(`operator ${mode} cancels clipboard export without leaking detail on reopen`, async ({
    page,
  }) => {
    const detail = {
      id: "report",
    ...
    await page.route("**/api/**", (route) => {
      ...
      if (path === "/api/operator/feedback/report")
        return route.fulfill({
          json: new URL(route.request().url()).searchParams.has("reveal")
            ? { ...detail, contact: "PRIVATE-TEST-CONTACT" }
            : detail,
        });
    ...
    await dialog
      .getByRole("button", { name: "Показать чувствительные данные" })
      .click();
    await expect(
      dialog.getByText("PRIVATE-TEST-CONTACT", { exact: true }),
    ).toBeVisible();
    await dialog
      .getByRole("button", {
    ...
    await pending!.fulfill(
      mode === "revoked"
        ...
        : { json: { description: "REDACTED-EXPORT" } },
    );
    await delivered;
    await expect(dialog).toHaveCount(0);
    await expect(
      page.getByText("PRIVATE-TEST-CONTACT", { exact: true }),
    ).toHaveCount(0);
    await openWorkspaceSection(page, "Обратная связь");
    await expect(dialog.getByRole("button", { name: /Ошибка/ })).toBeVisible();
    await expect(dialog.getByText(detail.title, { exact: true })).toHaveCount(
      0,
    );
    await expect(

...
function snapshotFor(role: "GM" | "PLAYER"): GameSnapshot {
  ...
  return {
    ...
    me: {
      id: membershipId,
      role,
      displayName: role === "GM" ? "Master" : "Player",
      characterId: null,
    },
    members: [
      { id: ids.gm, role: "GM", displayName: "Master", characterId: null },
      {
        id: ids.player,
        role: "PLAYER",
        displayName: "Player",
...
function adminPost(
  ...
  return {
    id: ids.post,
    threadId: ids.storyThread,
    authorMembershipId: ids.gm,
    title: "Arrival at Ravenford",
    body,
    lifecycle,
    revision,
    entityLinks: [],
...
async function mockApp(
  ...
  getPosts: () => Array<StoryPostDto | StoryPostAdminDto>,
) {
  ...
  await page.route("**/api/story/posts?*", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ posts: getPosts(), nextCursor: null }),
    }),
  );
}
...
test("GM drafts, publishes, corrects and archives a story post through refreshed channel data", async ({
  ...
  await page.route(`**/api/story/posts/${ids.post}/publish`, async (route) => {
    expect(route.request().method()).toBe("POST");
    const revision = (route.request().postDataJSON() as { revision: number })
      .revision;
  ...
  await page
    .locator(".story-composer__gm-notes textarea")
    .fill("GM-only preparation note");
  await body.focus();
  ...
  await post
    .locator(".story-post__edit textarea")
    .first()
    .fill("The gates open for the whole party.");
  await post
    .locator(".story-post__edit textarea")
    .last()
    .fill("Updated GM-only note");
